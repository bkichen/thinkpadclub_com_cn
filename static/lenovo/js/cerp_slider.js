...
function worker() {
    ...
    this.init = function (obj) {
        ...
        //jQuery(obj.renderTo).myslider(this.initObj);
    ...
    this.regEventCancel=function () {
        if(that.btnCancel!=""){
            jQuery(document).delegate(that.btnCancel,"click",function () {
                jQuery(that.triggerDivId).myslider(that.initObj);
            });
        }
    }
        ...
        if (result) {
            ...
            if(this.debug != undefined){
                if(this.debug == "success"){
                    jQuery(that.triggerDivId).myslider(that.initObj,"renderSuccess");
                    _this.success({"value": "333", "codeid": "111"});
                }else{
                    jQuery(that.triggerDivId).myslider(that.initObj,"renderError");
